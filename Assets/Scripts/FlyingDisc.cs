...
public class FlyingDisc : MonoBehaviour
{
    ...
    {
        // Define drag
        Vector3 dragForce = (-drag * _velocity) * Time.deltaTime;

        // Define lift
        Vector3 nForce = _velocity.sqrMagnitude * lift * Vector3.Project(-Physics.gravity, DiscHandle.up.normalized).normalized;
        ...
        // Curve mechanics
        Vector3 sideDir = Vector3.Cross(DiscHandle.up, _velocity).normalized;
    ...
    private void MoveDisc()
    {
        ...
        float rotSpeed = (-curveAmount * _OrientSpeed) / _velocity.magnitude;
